...
class nnInteractiveInferenceSession():
    def __init__(self,
        ...
        self.image_properties: dict = None
    ...
    @torch.inference_mode()
    def _predict_foreground_prob_once(self, prediction_center, zoom_out_factor: float = 1.0) -> torch.Tensor:
        assert self.pad_mode_data == 'constant', 'pad modes other than constant are not implemented here'
        input_for_predict, scaled_patch_size, scaled_bbox = self._build_network_input(prediction_center, zoom_out_factor)
        with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
        ...
        fg_prob = probs[1]
        if not all([i == j for i, j in zip(fg_prob.shape, scaled_patch_size)]):
            fg_prob = interpolate(fg_prob[None, None].to(float), scaled_patch_size, mode='trilinear')[0, 0]
        full_prob = torch.zeros_like(self.interactions[0], device='cpu', dtype=torch.float32)
        paste_tensor(full_prob, fg_prob.to('cpu'), scaled_bbox)
        empty_cache(self.device)
        return full_prob

    @torch.inference_mode()
    def export_multilabel_from_point_prompts(self,
                                             point_prompts: List[Tuple[Tuple[int, int, int], bool]],
                                             class_order: List[int],
                                             output_nii_path: str,
                                             background_threshold: Optional[float] = None):
        """
        Run multiple point prompts (grouped per class), fuse four foreground probability maps via argmax and
        export a multi-class NIfTI at output_nii_path. point_prompts is a list of (coords, is_positive) applied in
        order for the current class; call this method four times by grouping prompts per class externally is
        cumbersome, so instead we expect prompts per-class to be applied sequentially by resetting interactions per class.

        Args:
            point_prompts: A flat list applied per class in chunks; use class_order to delineate classes.
                          For simplicity, provide four chunks serially: for each class in class_order,
                          pass its list of (xyz, include) tuples; we reset between classes.
            class_order: Class label values to write (e.g., [1,2,3,4]). Must have length 4.
            output_nii_path: Destination .nii.gz path.
            background_threshold: If set, voxels with max prob < threshold are set to 0 (background).
        """
        assert len(class_order) == 4, 'class_order must have 4 labels'
        self._finish_preprocessing_and_initialize_interactions()
        ...
        for cls_idx in range(4):
            self.reset_interactions()
            class_points = point_prompts[cls_idx]
            last_center = None
            for coords, include in class_points:
                self.add_point_interaction(coords, include_interaction=include, run_prediction=False)
                last_center = [round(i) for i in transform_coordinates_noresampling(coords, self.preprocessed_props['bbox_used_for_cropping'])]
            if last_center is None:
                prob_maps.append(torch.zeros_like(self.interactions[0], dtype=torch.float32))
                centers.append([s//2 for s in self.interactions[0].shape])
            else:
                centers.append(last_center)
                prob_maps.append(self._predict_foreground_prob_once(last_center, zoom_out_factor=1.0))

        stacked = torch.stack(prob_maps, dim=0)  # (4, X, Y, Z)
        max_probs, winner = torch.max(stacked, dim=0)
        labels = torch.zeros_like(winner, dtype=torch.int16)
        for idx, cls_val in enumerate(class_order):
            labels[winner == idx] = int(cls_val)
        if background_threshold is not None:
            labels[max_probs < float(background_threshold)] = 0

        # uncrop back to original image space using stored bbox
        if self.preprocessed_props is not None and 'bbox_used_for_cropping' in self.preprocessed_props:
            bbox = self.preprocessed_props['bbox_used_for_cropping']
            full_shape = self.original_image_shape[1:]
            full_seg = torch.zeros(full_shape, dtype=labels.dtype)
            paste_tensor(full_seg, labels, bbox)
            labels = full_seg

        seg_np = labels.cpu().numpy()
        affine = None
        if self.image_properties is not None:
            affine = self.image_properties.get('affine', None)
        if affine is None:
            affine = np.eye(4)
        img = nib.Nifti1Image(seg_np.astype(np.int16), affine)
        nib.save(img, output_nii_path)
        print(f'Wrote multi-label segmentation to {output_nii_path}')

    def _find_penultimate_conv(self):
        net = self.network
        ...
        for name, m in net.named_modules():
            if isinstance(m, (nn.Conv1d, nn.Conv2d, nn.Conv3d)):
                convs.append((name, m))
        assert len(convs) >= 2, 'Model has fewer than 2 conv layers; cannot find penultimate.'
        return convs[-2]

    @torch.inference_mode()
    def export_multilabel_with_features(self,
                                        point_prompts_per_class: List[List[Tuple[Tuple[int, int, int], bool]]],
                                        class_order: List[int],
                                        output_nii_path: str,
                                        background_threshold: Optional[float] = None,
                                        logits_npy_path: Optional[str] = None,
                                        fuse_mode: str = 'concat'):
        """
        For each class, run point prompts once, capture penultimate-layer features via a forward hook,
        paste features into global feature maps (cropped space), sum across classes, map with 1x1x1 conv to 5 channels
        (background + 4 labels), uncrop back to original image space and save labels as NIfTI. Optionally save logits (numpy).

        Args:
            point_prompts_per_class: list of 4 classes; each class is a list of ((x,y,z), include_bool)
            class_order: label values to write for winner classes, e.g. [1,2,3,4]
            output_nii_path: path to output .nii.gz label map
            background_threshold: if set, pixels with max prob < threshold become background (0)
            logits_npy_path: optional path to save 5-channel logits as numpy array (cropped back to original space)
        """
        assert len(class_order) == 4, 'class_order must have 4 labels'
        assert len(point_prompts_per_class) == 4, 'Expect 4 classes of point prompts'
        self._finish_preprocessing_and_initialize_interactions()

        # Prepare hook on penultimate conv
        penult_name, penult_mod = self._find_penultimate_conv()
        bucket = {}
        ...
        def _hook(module, inp, out):
            ...
            if t.dim() == 5:
                t = t[0]
            bucket["feat"] = t.to(dtype=torch.float32, device='cpu')

        handle = penult_mod.register_forward_hook(_hook)

        # Accumulate per-class global features (cropped space)
        global_feats = []
        bbox_ref = None
        ...
        for cls_idx in range(4):
            self.reset_interactions()
            last_center = None
            for coords, include in point_prompts_per_class[cls_idx]:
                self.add_point_interaction(coords, include_interaction=include, run_prediction=False)
                last_center = [round(i) for i in transform_coordinates_noresampling(coords, self.preprocessed_props['bbox_used_for_cropping'])]
            if last_center is None:
                continue

            input_for_predict, scaled_patch_size, scaled_bbox = self._build_network_input(last_center, 1.0)
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                _ = self.network(input_for_predict[None])
            del input_for_predict

            feat = bucket.get('feat', None)
            if feat is None:
                continue
            # Ensure spatial match with patch size
            if tuple(feat.shape[-3:]) != tuple(self.configuration_manager.patch_size):
                feat = interpolate(feat[None], size=self.configuration_manager.patch_size, mode='trilinear', align_corners=False)[0]

            # Allocate per-class global feature map on CPU and paste
            if bbox_ref is None:
                bbox_ref = scaled_bbox
            tmp = torch.zeros((feat.shape[0], *self.interactions[0].shape), dtype=torch.float32)
            # paste_tensor expects 3D src; loop channels
            for c in range(feat.shape[0]):
                paste_tensor(tmp[c], feat[c], scaled_bbox)
            global_feats.append(tmp)

        handle.remove()

        assert len(global_feats) > 0, 'No features captured from any class prompts.'

        # Fuse features across classes
        fuse_mode = fuse_mode.lower()
        if fuse_mode == 'sum':
            global_feat = torch.stack(global_feats, dim=0).sum(dim=0)
        elif fuse_mode == 'concat':
            global_feat = torch.cat(global_feats, dim=0)
        else:
            raise ValueError(f"Unsupported fuse_mode '{fuse_mode}'. Use 'sum' or 'concat'.")

        # Map to 5 channels with a 1x1x1 conv (CPU)
        in_channels = global_feat.shape[0]
        head5 = nn.Conv3d(in_channels, 5, kernel_size=1, bias=True)
        with torch.inference_mode():
            logits5_cropped = head5(global_feat[None])[0].cpu()  # (5, Xc, Yc, Zc)

        # Uncrop logits back to original image space
        full_shape = self.original_image_shape[1:]
        logits5_full = torch.zeros((5, *full_shape), dtype=logits5_cropped.dtype)
        if self.preprocessed_props is not None and 'bbox_used_for_cropping' in self.preprocessed_props:
            bbox = self.preprocessed_props['bbox_used_for_cropping']
            for ch in range(5):
                paste_tensor(logits5_full[ch], logits5_cropped[ch], bbox)
        else:
            logits5_full = logits5_cropped

        probs5_full = torch.softmax(logits5_full, dim=0)
        max_probs, winner = torch.max(probs5_full, dim=0)
        labels = torch.zeros_like(winner, dtype=torch.int16)
        for idx, cls_val in enumerate(class_order):
            labels[winner == (idx + 1)] = int(cls_val)  # idx+1 because 0 reserved for background
        if background_threshold is not None:
            labels[max_probs < float(background_threshold)] = 0
        ...
        img = nib.Nifti1Image(seg_np.astype(np.int16), affine)
        nib.save(img, output_nii_path)
        print(f'Wrote multi-label (with features) segmentation to {output_nii_path}')

        # Optionally save logits as numpy
        if logits_npy_path is not None:
            np.save(logits_npy_path, logits5_full.cpu().numpy())
            print(f'Wrote 5-channel logits to {logits_npy_path}')
